...
var u = (r, o, s) => o in r ? m(r, o, { enumerable: !0, configurable: !0, writable: !0, value: s }) : r[o] = s;
var c = (r, o, s) => u(r, typeof o != "symbol" ? o + "" : o, s);
const G = (r, o) => r + o, X = () => Array(8).fill(0).map(() => Array(8).fill(0)), g = (r) => r.split("").map(
  (o) => isNaN(parseInt(o)) ? o : Array(parseInt(o)).fill(0)
).flat().filter((o) => o != "/").reduce(
  (o, s, t) => (t < 64 && o[Math.floor(t / 8)].push(s), o),
  [[], [], [], [], [], [], [], []]
), Y = () => g("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1"), U = (r) => {
  let o = [];
  for (let s of r) {
    let t = 0;
    for (let e of s)
      e != 0 && t > 0 && (o.push(t), t = 0), e != 0 && o.push(e), e === 0 && t++;
    t > 0 && o.push(t), o.push("/");
  }
  return o.pop(), o.join("");
}, b = (r) => ([o, s], [t, e]) => {
  const i = structuredClone(r), n = i[s][o];
  return i[s][o] = 0, i[e][t] = n, i;
...
class K extends HTMLElement {
  constructor() {
    ...
    c(this, "board");
    c(this, "divBoard");
    c(this, "chessImages", Object.fromEntries(
      Object.entries(F).map(([s, t]) => [
        `${s}`.split("/")[2].split(".")[0],
        t.default
      ])
    ));
    c(this, "blackColor", "#b58863");
    c(this, "whiteColor", "#f0d9b5");
    c(this, "numbers", !1);
    c(this, "numbersColor", "black");
    c(this, "legalMoves", []);
    this.board = Array(8).fill(0).map(() => Array(8).fill(0)), this.divBoard = document.createElement("div");
  }
  connectedCallback() {
    this.dataset.fen && (this.board = g(this.dataset.fen)), this.dataset.blackColor && (this.blackColor = this.dataset.blackColor), this.dataset.whiteColor && (this.whiteColor = this.dataset.whiteColor), this.dataset.numbers && (this.numbers = this.dataset.numbers === "true"), this.dataset.numbersColor && (this.numbersColor = this.dataset.numbersColor), this.dataset.legalMoves && (this.legalMoves = JSON.parse(this.dataset.legalMoves).map(J)), this.render();
  }
  ...
  refresh() {
    const s = this.divBoard;
    s.innerHTML = this.board ? (
      // Top Letters
      `${this.numbers ? a.map(
        (t) => `<div class="piece letters ${t != " " ? "" : "numbers"}">${t}</div>`
      ).join("") : ""}` + this.board.map(
        (t, e) => (
          //left numbers
          `${this.numbers ? `
          <div class="piece numbers">${8 - e}</div>
          ` : ""}` + // Pieces
          t.map(
            (i, n) => `
            <div class="piece
            ...
            ${(e + n) % 2 === 0 ? "white" : ""}
            "
            data-position="${a[n + 1]}${8 - e}"
            data-letter="${a[n + 1]}"
            data-number="${7 - e}"
            data-matrix_position="[${n},${e}]"
            >${i != 0 ? `<img data-piece="${i}" data-position="${a[n + 1]}${8 - e}"
            data-letter="${a[n + 1]}"
            data-number="${8 - e}"
            data-matrix_position="[${n},${e}]"  src="${this.chessImages[i]}">` : ""}</div>
            `
          ).join("") + // right numbers
          `${this.numbers ? `
          <div class="piece numbers">${8 - e}</div>
      ...
      `${this.numbers ? a.map((t) => `<div class="piece letters ${t != " " ? "" : "numbers"}">${t}</div>`).join("") : ""}`
    ) : "", s.querySelectorAll("div.piece").forEach((t) => {
      const e = t;
      e.addEventListener("drop", (i) => {
        var d;
        i.preventDefault();
        const n = ((d = i.dataTransfer) == null ? void 0 : d.getData("application/json")) || "", l = JSON.parse(n);
        this.dispatchEvent(
          new CustomEvent("chessmarro-move", {
            ...
            detail: {
              ...
              destiny: {
                ...
                matrix_position: e.dataset.matrix_position
              },
              uci: l.position + e.dataset.position
            }
          })
        );
    ...
    }), s.querySelectorAll("img").forEach((t) => {
      t.addEventListener("dragstart", (e) => {
        e.dataTransfer && e.target instanceof HTMLImageElement && (e.dataTransfer.setData("text/plain", e.target.dataset.piece), e.dataTransfer.setData(
          "application/json",
          JSON.stringify({
            piece: e.target.dataset.piece,
            position: e.target.dataset.position,
            matrix_position: e.target.dataset.matrix_position
          })
        ), console.log(t));
      });
    });
  ...
  render() {
    const s = this.shadowRoot ? this.shadowRoot : this.attachShadow({ mode: "open" }), t = document.createDocumentFragment(), e = this.divBoard;
    e.id = "divBoard", e.style.opacity = "0";
    ...
    }), s.adoptedStyleSheets = [i], this.refresh(), t.appendChild(e), s.replaceChildren(t), requestAnimationFrame(() => {
      e.style.opacity = "1";
    });
  }
  movePiece([s, t], [e, i], n = 0.5) {
    const l = this.shadowRoot.querySelector(
      `img[data-position="${a[s + 1]}${8 - t}"]`
    ), d = this.shadowRoot.querySelector(
      `div[data-position="${a[e + 1]}${8 - i}"]`
    );
    if (l && d) {
      const h = l.getBoundingClientRect(), p = d.getBoundingClientRect(), v = p.left - h.left, f = p.top - h.top;
      l.style.transition = `transform ${n}s ease-in-out`, l.style.transform = `translate(${v}px, ${f}px)`, l.addEventListener(
        "transitionend",
        () => {
          this.board = b(this.board)([s, t], [e, i]), this.refresh();
        },
        { once: !0 }
      );
    }
